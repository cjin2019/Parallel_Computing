...
//
void diff( triple* t , triple r , triple n ) // t = u - v
{
   double dot = dotp(r, n);
   t->x = r.x - 2 * dot * n.x;
   t->y = u.y - v.y ;
...
//
void reflect (triple* t, triple u, triple v){
   t -> x = 
}
int main(int argc, char const *argv[])
{
   ...
   for( y = 0 ; y < N ; y++ )
   {
      for( x = 0 ; x < M ; x++)
      {
         ...
         //gives the checkerboard
         if(sNum == 0){
            int xi = (int )(ptSphere.x/0.1+1000);//divide by 0.1 gives the size of the square
            int zi = (int )(ptSphere.z/0.1+1000);
            if((xi+zi)%2==0){
               rgb[y][x][0] = 103  ; // red
               rgb[y][x][1] = 0    ; // green
               rgb[y][x][2] = 255  ; // blue
