...
//
void createPic( triple e, int dimness, int shad){
   sphere a[4];
   a[0].c.x =      0.50 ;
   ...
   for( y = 0 ; y < N ; y++ )
   {
      for( x = 0 ; x < M ; x++)
      {
         ...
         triple l = {ptSphere.x-g.x, ptSphere.y-g.y, ptSphere.z-g.z};
         ...
         for(int i = 0; i < 4; i++){
            if(i==sNum)
               continue;
            triple d = {a[i].c.x - g.x, a[i].c.y - g.y, a[i].c.z - g.z};
            double A = 1.0;
